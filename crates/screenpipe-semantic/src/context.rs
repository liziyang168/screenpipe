...
pub fn render_semantic_context(
    ...
    for item in items {
        ...
        for value in [item.title.as_deref(), item.actor.as_deref()] {
            if let Some(value) = value.map(inline_text).filter(|value| !value.is_empty()) {
        ...
        if body.is_empty() {
            ...
            let _ = writeln!(output, " | {}", inline_text(body));
        } else {
            output.push('\n');
...
mod tests {
    ...
    fn renders_relationships_and_omits_internal_fields() {
        ...
        assert!(context.contains("conversation | #release"));
        assert!(context.contains("  message | Alice | release is blocked"));
